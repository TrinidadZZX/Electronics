...
u8 OV7670_Init(void)
{
    ...
    //设置 IO
    RCC->APB2ENR|=1<<2;		//先使能外设 PORTA时钟
    RCC->APB2ENR|=1<<3;		//先使能外设 PORTB时钟
    RCC->APB2ENR|=1<<7;		//先使能外设 PORTF时钟

    GPIOA->CRL=0X00030000;   //PA4 输出
    GPIOA->CRH=0X83033033; 	//PA15 输入、PA11/12/14输出

    SCCB_Init();        		            //初始化SCCB的 IO口
